...
const AppProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        return () => { unsubRecords(); unsubUsers(); unsubNotifications(); };
    ...
    const value = { db, app, isAuthReady, isLoginDataReady, records, users, showToast, currentUser, setCurrentUser, userRole, setUserRole, loginStep, setLoginStep, activityNotifications, recordToAutoEdit, setRecordToAutoEdit, lastSaveTimestamp, setLastSaveTimestamp, postSaveAction, setPostSaveAction, modal, openModal, closeModal };
...
const useFirestore = () => {
    ...
    const deleteRecordPermanent = async (recordId, name) => {
        ...
        try {
            ...
            console.error("Error permanent delete:", error); // Log error for debugging
        }
    };
    ...
    return { addOrUpdateRecord, softDeleteRecord, restoreRecord, deleteRecordPermanent, addOrUpdateUser, deleteUser, mergeMasterData };
};
...
// Hook baru untuk logika laporan
const useReports = (onEditParticipant, activeRecords) => {
    ...
    const handleConfirmBulkDelete = useCallback(async (idsToDelete) => {
        ...
        try {
            ...
            openModal(null); // Close all modals
        } catch (error) {
...
const AttendancePopup = ({ onClose, tcParticipants, athleticParticipants, seitaiParticipants, locations }) => {
    ...
    const firestore = useFirestore();
    const [activeList, setActiveList] = useState('TC'); // TC, Athletic, Seitai
            ...
            <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
                <div className="popup-wrapper popup-wrapper-visible bg-gray-800 p-6 rounded-xl shadow-neumorphic w-full max-w-2xl border-2 border-green-500 flex flex-col" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex-grow overflow-y-auto max-h-[50vh] pr-2 space-y-4">
                        {currentParticipants.length > 0 ? currentParticipants.map(p => (
...
const TrainingReflexAthleticForm = ({ formValues, handleFormInputChange }) => (
    <>
...
const TrainingSeitaiForm = ({ formValues, handleFormInputChange }) => (
    <>
...
const CekingForm = ({ formValues, setFormValues, handleFormInputChange, onOpenAssessment, activeRecords }) => {
...
const DynamicForm = ({
    ...
    nameSuggestions, setNameSuggestions, onOpenAssessment, isSpecialTrainingPath
}) => {
    
    const renderFormContent = () => {
        const { status } = formValues;
        const formProps = { formValues, setFormValues, handleFormInputChange, onOpenAssessment, activeRecords };
...
const PesertaScreen = ({ records, allRecords, onEdit, onDelete, onUpdateLatest }) => {
    ...
    // Peserta yang belum Lulus atau Evaluasi akan ditampilkan di layar ini.
    ...
    const participantGroups = useMemo(() => {
        ...
        // Find all participant names that have ever been in specific training paths
        ...
        recordsForPesertaView.forEach(record => {
            ...
            if (isOnSeitaiPath) {
                seitaiGroup.push(record);
            } else if (isOnAthleticPath) {
                athleticGroup.push(record);
            } else if (record.trainganDari) {
                if (!tcGroups[record.trainganDari]) {
                    tcGroups[record.trainganDari] = [];
                }
    ...
    return (
        <main className="flex-grow overflow-y-auto p-4 md:p-8 pt-4 flex flex-col gap-8">
            {participantGroups.map(({ name, participants }, index) => {
                 ...
                 const filterOptions = isSpecialTrainingGroup
                     ? [
                           { value: "semua", label: "Tampilkan Semua" },
                           { value: "Trainingan", label: "Training" },
                           { value: "Ceking", label: "Ceking" },
                           { value: "Ganti Peserta", label: "Ganti Peserta" },
                       ]
                     : [
                           { value: "semua", label: "Tampilkan Semua" },
                           { value: "Trainingan", label: "Training" },
                    ...
                    <AnimateOnScroll key={name}>
                        <div className={`bg-gray-800 rounded-xl shadow-neumorphic border-2 ${colors[index % colors.length]} p-4 sm:p-6 space-y-4`}>
                            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                                ...
                                <select value={locationStatusFilters[name] || 'semua'} onChange={(e) => setLocationStatusFilters(prev => ({ ...prev, [name]: e.target.value }))} onClick={(e) => e.stopPropagation()} className="select-rounded-border !py-2 !px-3 !text-sm w-full sm:w-48">
                            ...
                            <div className="flex overflow-x-auto gap-4 p-2 bg-gray-900/50 rounded-lg horizontal-scroll-container">
                                {participants.filter(r => matchesStatusFilter(r, locationStatusFilters[name])).map(record => (
...
const CabangScreen = ({ records, allRecords, onEdit, onDelete, onUpdateLatest }) => {
    ...
    const recordsForCabangView = useMemo(() => {
        return records.filter(record => 
            record.status === 'Lulus' || record.status.startsWith('Evaluasi')
        );
    }, [records]);

    const relevantCabangs = useMemo(() => {
        const counts = recordsForCabangView
            .reduce((acc, record) => {
            ...
            {relevantCabangs.map((cabang, index) => (
                <AnimateOnScroll key={cabang}>
                    <div className={`bg-gray-800 rounded-xl shadow-neumorphic border-2 ${colors[index % colors.length]} p-4 sm:p-6 space-y-4`}>
                        ...
                        <div className="flex overflow-x-auto gap-4 p-2 bg-gray-900/50 rounded-lg horizontal-scroll-container">
                           {recordsForCabangView.filter(r => (r.turunKeCabang === cabang || r.cabang === cabang) && matchesEvaluationCategoryFilter(r, evaluationCategoryFilters[cabang])).map(record => (
                                <CekingCard key={record.id} record={record} onCardClick={handleCardClick} />
                            ))}
                        </div>
                         {expandedDetail && recordsForCabangView.some(r => r.id === expandedDetail.id && (r.turunKeCabang === cabang || r.cabang === cabang)) && (
                            <div className="w-full">
                                <ParticipantDetailView
                                    participant={expandedDetail}
                                    allRecords={allRecords}
        ...
        openModal('mergeData', {
            ...
            onConfirm: async () => {
                ...
                if (success) {
                    ...
                    closeModal(); // Close master data management
                }
            }
        });
    };
...
const NavMenu = ({ setActiveView, onEditParticipant, activeRecords }) => {
    ...
    const { attendanceParticipants, allFilterOptions, allRecords } = useRecords('', 'semua');
        ...
        <div ref={navMenuRef} className="relative">
            ...
            {isNavMenuOpen && (
                <div className="absolute top-full right-0 mt-2 w-64 bg-gray-700 rounded-lg shadow-lg z-20 border border-gray-600 p-2 space-y-1">
                   ...
                   {currentUser?.permissions?.daftarHadir && <button onClick={() => handleMenuClick(handleOpenAttendance)} className="w-full text-left px-4 py-2 rounded-md font-semibold text-white bg-emerald-500 hover:bg-emerald-600">Daftar Hadir Peserta</button>}
...
// =================================================================================
function AppContent() {
  const { currentUser, showToast, activityNotifications, setLastSaveTimestamp, postSaveAction, setPostSaveAction, openModal } = useContext(AppContext);
  const firestore = useFirestore();
  ...
  const initialFormState = useMemo(() => ({
      nama: '', tgl: '', trainer: '', status: 'Training Reflexology', tahapCeking: '',
  ...
  const { formValues, setFormValues, recordToEdit, setRecordToEdit, nameSuggestions, setNameSuggestions, handleFormInputChange, handleSuggestionClick, resetForm } = useForm(initialFormState, uniqueLatestRecords, allRecords);
    ...
    const isSpecialTrainingPath = useMemo(() => {
        ...
        // Check history based on name in form or record being edited
  ...
  const handleUpdateLatestClick = useCallback((record) => {
    ...
    setIsFormExpanded(true);
    if (addFormRef.current) addFormRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, [allRecords, initialFormState, setFormValues, setRecordToEdit]);
...
function App() {
    useEffect(() => {
        ...
        // --- PENAMBAHAN DINAMIS UNTUK MANIFEST DAN IKON ---
        if (!document.getElementById('app-manifest-link')) {
            const manifestContent = {
              ...
              "theme_color": "#111827",
              "icons": [
                { "src": "https://i.ibb.co/jvSJGBCt/512x512.jpg", "sizes": "72x72", "type": "image/jpeg" },
                { "src": "https://i.ibb.co/jvSJGBCt/512x512.jpg", "sizes": "96x96", "type": "image/jpeg" },
                { "src": "https://i.ibb.co/jvSJGBCt/512x512.jpg", "sizes": "128x128", "type": "image/jpeg" },
                { "src": "https://i.ibb.co/jvSJGBCt/512x512.jpg", "sizes": "144x144", "type": "image/jpeg" },
                { "src": "https://i.ibb.co/jvSJGBCt/512x512.jpg", "sizes": "152x152", "type": "image/jpeg" },
                { "src": "https://i.ibb.co/604jKG34/192x192.jpg", "sizes": "192x192", "type": "image/jpeg", "purpose": "any maskable" },
                { "src": "https://i.ibb.co/jvSJGBCt/512x512.jpg", "sizes": "384x384", "type": "image/jpeg" },
                { "src": "https://i.ibb.co/jvSJGBCt/512x512.jpg", "sizes": "512x512", "type": "image/jpeg" }
              ]
            };
            ...
            if (!appleIcon) {
                ...
                appleIcon.href = 'https://i.ibb.co/604jKG34/192x192.jpg';
                document.head.appendChild(appleIcon);
            }

            let favicon = document.querySelector('link[rel="icon"]');
            if (favicon) {
                favicon.href = 'https://i.ibb.co/604jKG34/192x192.jpg';
            } else {
